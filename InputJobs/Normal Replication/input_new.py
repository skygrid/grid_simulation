...
INPUT_SIZE_BY_TYPE = {"USER":31710*10**6, "DATASTRIPPING":4273*10**6, "MERGE":2072*10**6, "MCStripping":2.5*10**9, "DATARECONSTRUCTION":2887*10**6, "TURBO":2*10**9,  "MCRECONSTRUCTION":4429*10**6, "WGPRODUCTION":10**9, "MCMERGE":3857*10**6, "UNKNOWN":0, "MCSIMULATION":0, "TEST":0.1*10**9}
...
def fill_array(dataset_name, depth):
	...
	if len(inp_indexes) > 0:
		...
		if NREpIn == 2:
			location_str = ",".join(Locations[:prob_array_input[inp_indexes[0]]]) + ",0,0,0,0,0,0,0,0"
			storage_types = "1,0,none,none,none,none,none,none,none,none"
		elif NREpIn == 3:
			location_str = ",".join(Locations[:prob_array_input[inp_indexes[0]]]) + ",0,0,0,0,0,0,0"
			storage_types = "1,1,0,none,none,none,none,none,none,none"
		else:
			location_str = ",".join(Locations[:prob_array_input[inp_indexes[0]]]) + ",0,0,0,0,0,0"
			storage_types = "1,1,1,0,none,none,none,none,none,none"
...
for i in range(job_amount):
	...
	if types[types_custm[i]] == "MERGE":
		output_locations = "CERN1,CERN0," + ",".join(Location_DISK[:3]) + "," + Location_TAPE[0] + ",0,0,0,0"
	elif types[types_custm[i]] == "MCMERGE":
		output_locations = "CERN1,CERN0," + ",".join(Location_DISK[:2]) + "," + Location_TAPE[0] + ",0,0,0,0,0"
	elif types[types_custm[i]] == "USER":
		output_locations = ",".join(Location_DISK[:3]) + ",0,0,0,0,0,0,0"
	else:
