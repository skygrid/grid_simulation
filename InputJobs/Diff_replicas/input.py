...
replica_amount = int(sys.argv[2])
print("replicas: " + sys.argv[2])
print job_amount
...
def fill_array(dataset_name, depth):
	...
	if len(inp_indexes) > 0:
		...
		for item in inp_indexes:
			...
			NREpIn = replica_amount
			location_str = ",".join(Locations[:NREpIn]) + "," + str(Locations[0]) + "," + ",".join(["0"]*(9-NREpIn))
			storage_types = ",".join("1"*NREpIn) + ",0," + ",".join(["none"]*(9-NREpIn))
