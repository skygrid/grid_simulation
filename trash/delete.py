...
def create_correct_array(dataset_amount, time_array,  end_time):
    time_interval = np.arange(0, end_time+86400, 86400)
    true_array = np.array([])
...
def true_number_of_datasets(filename1, filename2):
	...
	tier_names, variable, timex, number_dset = np.loadtxt(filename1, delimiter=", ", usecols=(1, 2, 3, 6), unpack=True,
														  dtype=rtype)
	tier_names_DAM, variable_DAM, timex_DAM, number_dset_DAM = np.loadtxt(filename2, delimiter=", ",
	...
	time_interval = np.arange(0, timex[-1] + 86400, 86400)
	...
	time_interval = np.arange(0, timex_DAM[-1] + 86400, 86400)
	x = []
...
def number_of_datasets(filename1, filename2):
    ...
    tier_names, variable, timex, number_dset = np.loadtxt(filename1, delimiter=", ", usecols=(1,2,4,6), unpack=True, dtype=rtype)
    tier_names_DAM, variable_DAM, timex_DAM, number_dset_DAM = np.loadtxt(filename2, delimiter=", ", usecols=(1,2,4,6), unpack=True, dtype=rtype)
...
if __name__ == '__main__':
    true_number_of_datasets("trace.csv", "trace_DAM.csv")
    #cumulative_input_and_output_per_site("trace.csv", "trace_DAM.csv")
